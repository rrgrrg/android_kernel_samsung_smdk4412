				    ...
				    const char *buf, size_t len)
{
	if (sscanf(buf, "%d\n", &vibrator_level) == 1) {
                if (vibrator_level < 0) {
			// nothing to do here
		} else if (vibrator_level >= ARRAY_SIZE(vibrator_duty_levels)) {
...
static int __devinit vibrator_probe(struct platform_device *pdev)
{
	...
	error = device_create_file(ddata->dev.dev, &dev_attr_vibrator_level_max.attr);
	if (error < 0)
		pr_err("[VIB] create sysfs fail: pwm_value\n");

	error = device_create_file(ddata->dev.dev, &dev_attr_vibrator_level.attr);
	if (error < 0)
		pr_err("[VIB] create sysfs fail: pwm_value\n");
